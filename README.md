...
- **Config at runtime** — loops read their deployment instead of having it baked in: `agent.config()` (own profile incl. `goal` + extras, secrets as opaque markers), `runtime.triggers()` (the merged triggers list — replaces baked CRON_TASKS/ROUTES tables), and `credential.get(name)` (allow-listed, logged, counted; the value never touches the journal).
