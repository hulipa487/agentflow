...
// ParseEvery parses an every: interval: a duration in ms, s, m, h, or d
// units, optionally compound ("90s", "15m", "6h", "1d", "2d12h") with
// fractional values allowed ("1.5h"). Go's other duration units are not
// accepted — the smallest interval the service schedules is minEvery, so an
// odd unit is a typo worth naming rather than silently honouring.
